...
public class BottomsUp2 extends PApplet {
	...
	public void draw() {
		...
		if (random(0,100) < settings.getPercent()) {
			system.addBeer(new PVector(random (0, settings.getScreenWidth()), 0));
		}
	...
	class Beer {
		...
		public void update() {
			...
			if (position.y + settings.getGlassHeight() / 2 > height) {
				fallen = true;
	...
	class ParticleSystem {
		...
		public void initialize() {
			...
			for (int i = 0; i < numOfParticles; i++) {
				addBeer(new PVector(i*k+doorWidth, 0));
			}
